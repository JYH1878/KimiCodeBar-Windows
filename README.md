...
- **月度总量（可选）**：Kimi + Code 每月总用量分列显示（需在设置中粘贴一次网页 token）
- **本地消耗统计**：扫描本地 `wire.jsonl` 会话日志，按天统计 token 消耗（今日/昨日 + 近 7 天柱状图 + 模型占比），不依赖 API
...
- **全局热键**：任意界面一键唤起/收起面板（如 `Ctrl+Shift+K`，可自定义，默认关闭）
...
<p>
  <img src="docs/screenshots/panel.png" width="280" alt="用量面板">
  &nbsp;&nbsp;
  <img src="docs/screenshots/settings.png" width="405" alt="设置页">
</p>
